...
tumorID_list = c( "RCC" , "Melanoma" , "NSCLC", "UC" , "GBM", "GC")

for (tumorID in tumorID_list){
    if (tumorID=="GC"){next}###the os of GC is NULL
    res <- NULL
    for( i in 1:length(all_signatures)){
      file = paste( "./TLS_project/results/siganatures_ICB_per_cancer/" , all_signatures[ i ] ,"/",  tumorID , "/Overall/Response/",  all_signatures[ i ], "_Log_Response.RData" , sep="" )
      if( file.exists( file ) ){
        load( file )
    ...
    res = NULL
    for( i in 1:length(all_signatures)){
      file = paste("./TLS_project/results/siganatures_ICB_per_cancer/" , all_signatures[ i ] ,"/",  tumorID , "/Overall/OS/",  all_signatures[ i ], "_COX_OS.RData" , sep="" )
      if( file.exists( file ) ){
        load( file )
        res = rbind( res , c( meta_res , "sig" ) )
      }
    ...
    pheatmap( t( data[ order( rowSums(data)) , ] ) , cluster_rows=FALSE , cluster_cols=FALSE , scale="none" , annotation_col = annotation_col, annotation_colors = ann_colors, 
          ...
          filename=paste0("./TLS_project/results/summary_figures/heatmap_per_tumor/", tumorID, "_Heatmap_Summary_ICB.pdf"), height=2.8, width=9.5 )  
    ...
    pheatmap( t( data[ order( rowSums(data)) , ] ) , cluster_rows=FALSE , cluster_cols=FALSE , annotation_col = annotation_col, annotation_colors = ann_colors, 
          col = col , breaks = c( neg , pos ) , na_col="white" , border_color="#424242", angle_col = 315, fontfamily="serif" ,
          #display_numbers = data , number_color="black" , fontsize_number= 10 , 
          filename=paste0("./TLS_project/results/summary_figures/heatmap_per_tumor/", tumorID, "_Heatmap_Summary_legend_ICB.pdf"),  height=4, width=9.5 )  
}
